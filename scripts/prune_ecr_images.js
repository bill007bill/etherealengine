...
const getAllPods = async(k8Client, continueValue, labelSelector, pods = []) => {
  ...
  if (matchingPods.body.metadata?._continue) return getAllPods(k8Client, matchingPods.body.metadata._continue, labelSelector, pods)
  else return pods
}
...
const getAllImages = async(ecr, repoName, token, images=[]) => {
  ...
  if (response.nextToken) return getAllImages(ecr, repoName, response.nextToken, images)
  else return images
}
...
const deleteImages = async(ecr, toBeDeleted) => {
  ...
  if (toBeDeleted.length >= 100) return deleteImages(ecr, toBeDeleted.slice(100))
